...
def create_left_prompt [] {
    ...
    let dir = (
        if ($env.PWD | path split | zip ($home | path split) | all { $in.0 == $in.1 }) {
            ($env.PWD | str replace $home "\u{301C}")
        } else {
            $env.PWD
        }
    )
    ...
    let path_segment = $"(ansi white_bold)\u{3010}(ansi reset)($path_color)($dir)(ansi reset)(ansi white_bold)\u{3011}"
...
} {neofetch --ascii_colors 11 7 --colors 14 4 15 6}
